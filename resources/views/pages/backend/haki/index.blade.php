            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header d-flex align-items-center justify-content-end">
                        <a href="{{ route('haki.create') }}">
                            <button class="btn btn-primary btn-xs"><i class="fa fa-plus"></i> HAKI</button>
                        </a>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body">
                        <table id="hakis-table" class="table table-bordered">
                            <thead>
                                <tr>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ $haki->member->name }}</td>
                                    <td>{{ $haki->member->nip }}</td>
                                    <td>{{ $haki->major->name }}</td>
                                    ...
                                    <td>
                                        <a href="{{ route('haki.edit', $haki) }}"><button class="btn btn-info btn-xs"><i
                                                    class="fa fa-edit"></i></button></a>
                                        <button class="btn btn-danger btn-xs" id="btn-delete" data-toggle="modal"
                                            data-target="#modal-delete" data-url="{{ route('haki.destroy', $haki) }}"
                                            data-register="{{ $haki->register_number }}"><i
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Deleting haki</h4>
            </div>
            <form id="form-delete" method="POST">
                @csrf
                @method('DELETE')
                <div class="modal-body">
                    <p class="text-center">Do you want to delete "<span id="haki-title" class="text-danger"></span>"
                        ?
                    </p>
                </div>
...
<script>
    $(document).ready(function () {
        $('#hakis-table').DataTable({
            "responsive": true,
        });

        $('#modal-delete').on('show.bs.modal', function (e) {
            const url = $(e.relatedTarget).data('url');
            const register = $(e.relatedTarget).data('register');
            $('#form-delete').attr('action', url);
            $('.modal-body #haki-title').text(register);
        });
